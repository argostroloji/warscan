...
export class DeckGLMap {
  ...
  private setupDOM(): void {
    ...
    // Bounty placement handler
    wrapper.addEventListener('contextmenu', (e) => {
      e.preventDefault();

      // Ensure MapLibre has calculated bounds properly
      if (!this.maplibreMap) return;

      // Get lng/lat from screen coordinates
      const rect = wrapper.getBoundingClientRect();
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;
      const point = this.maplibreMap.unproject([x, y]);

      // Trigger generic custom event so the App or PanelLayout can catch it and open the prompt
      wrapper.dispatchEvent(new CustomEvent('map-bounty-click', {
        detail: {
          lat: point.lat,
          lng: point.lng
        },
